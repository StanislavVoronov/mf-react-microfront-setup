...
Реестр в entry берётся обычным `fetch` — точка входа обязана оставаться
свободной от React-импортов. TanStack Query подключается уже в UI
([App.tsx](mf-host/src/App.tsx)): ему принадлежит состояние загрузки, ошибок
и кнопка «Повторить».
